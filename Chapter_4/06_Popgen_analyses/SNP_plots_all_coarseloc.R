...
setwd("/nesi/nobackup/ga03488/David_H/Ppo_Popgen/14_whole_genome_snpcalling/05_SNP_all/")
...
sample_valleys <- as.data.frame(read_tsv("my.strata_locations_coarse.tsv"))
...
load("data_variants_WG_inclmissing.RData")
...
print("finished PCA")
#save(ArthursPass_vcf_80.gl, ArthursPass_vcf_80.gid, ArthursPass_vcf_80.pca, file = "data_variants_WG_inclmissing.RData")
...
nei_dist <- nei.dist(ArthursPass_vcf_80.gid, warning = FALSE)
nei_dist <- prevosti.dist(ArthursPass_vcf_80.gid)
...
#poppr_p <- poppr(ArthursPass_vcf_80.gid, sample = 99, plot = FALSE, quiet = FALSE) 
#poppr_p <- poppr(ArthursPass_vcf_80.gid, sample = 9, plot = FALSE, quiet = FALSE, missing = "loci") 

# Save data
#save(amova_result, poppr_p, file = "data.RData")

# Saving to PDF
pdf(paste("Plots_all_coarse_nosub_", basename_vcf, ".pdf", sep=""))
eigenplot
print(plot12)
print(plot34)
print(plot12ellipse)
...
#plot(amova.test)
grid.newpage()  # Force a new page
grid.text(paste(mantel_text, collapse = "\n"), x = 0.1, y = 0.9, just = "left", gp = gpar(fontsize = 8), check.overlap=TRUE)
