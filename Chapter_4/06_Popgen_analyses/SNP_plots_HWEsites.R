...
my_vcf <- "C:/Users/herad/OneDrive - MWLR/Fungi, Food and Fodder/03 - Molecular work/PopGen Bioinformatics/04_WG_SNPs/metrics/WG_all_hwe_deviating_sites.vcf"  # First argument passed to the script
...
# Load strata
strata(ArthursPass_vcf_80.gid) <- sample_valleys[-13,]
pop(ArthursPass_vcf_80.gl) <- sample_valleys_only[-13]
pop(ArthursPass_vcf_80.gid) <- sample_valleys_only[-13]
...
ArthursPass_vcf_80.pca.scores$site <- sample_valleys_only[-13]
...
#load("data.RData")
amova_result <- poppr.amova(ArthursPass_vcf_80.gid, ~POPULATION, within = TRUE)
