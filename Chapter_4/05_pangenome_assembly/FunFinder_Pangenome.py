...
if not analyses == 'none':
    ...
    for dir in input_directory:
        ...
        for a in analyses:
            ...
            #found_file = [files for files in os.listdir(dir) if a in files.lower() and not files.startswith('.')]
            ...
            if a == 'antismash':
                ...
                sys.exit()
        found_file = [files for files in os.listdir(dir) if 'protein' in files]
        if not found_file:
...
def create_ortho_dictionaries(orthogroups_text_file):
    ...
    for cluster in ortho_iso_dict.keys():
        ...
        #if count >= 0.95 * iso_num:
        if count == iso_num:
            pan_cluster_dict['Core'].append(cluster)
        # elif 0.95 * iso_num > count > 1:
...
def create_pangenome_stats(sortCount,stats_text_out,stats_fig_out,iso_per_cluster,clusters_per_PAN,proteins_per_PAN):
    ...
    with open(stats_text_out, 'w') as stats_out:
        ...
        #core_threshold = 27  # Calculate the 95% threshold
        #Core = sum([x[1] for x in sortCount if x[0] >= core_threshold])
        #Accessory = sum([x[1] for x in sortCount if core_threshold > x[0] > 1])
        #Singleton = sum([x[1] for x in sortCount if x[0] == 1])
        Core = sortCount[0][1]
    ...
    for i in range(0, len(c_bar)): # loop through bars for specific colors
        if i == 0:
            ...
            c_bar[i].set_edgecolor('black')
    plt.xlim(-1,iso_num)
    plt.xlabel('Number of genomes', fontname='DejaVu Sans', fontsize=12)
    plt.ylabel('Number of protein clusters', fontname='DejaVu Sans', fontsize=12)
...
def determine_pangenome_fluidity(fluidity_figure, fluidity_results):
    ...
    plt.xlabel('Number of genomes sampled', fontname='DejaVu Sans')
    plt.ylabel('Fluidity, '+u'\u03C6', fontname='DejaVu Sans')
    plt.tight_layout()
...
def determine_pangenome_curve(curve_figure, curve_results):
    ...
    plt.xlabel('Number of genomes sampled', fontname='DejaVu Sans', fontsize=12)
    plt.ylabel('Number of gene clusters', fontname='DejaVu Sans', fontsize=12)
...
def determine_pangenome_protein_lengths(length_figure, protein_lengths):
    ...
    for tick, label in zip(pos, kruskal_labels): # loop through and place significance letters above median position
        ...
        label, ha='center', va='bottom', fontsize=12, fontname='DejaVu Sans')
    plt.xlabel('Pangenome category', fontname='DejaVu Sans', fontsize=12)
    plt.ylabel('Protein length (aa)', fontname='DejaVu Sans', fontsize=12)
...
def parse_phobius(infile, secret_dict, trans_dict, isolate_dir):
    phobius_secreted_output = os.path.abspath(os.path.join(isolate_dir, 'phobius_secreted.txt'))
    phobius_transmembrane_output = os.path.abspath(os.path.join(isolate_dir, 'phobius_transmembrane.txt'))

    with open(infile, 'r') as files, open(phobius_secreted_output, 'w') as ph_s_out, open(phobius_transmembrane_output, 'w') as ph_t_out:
        for line in files:
            if not 'PREDICTION' in line.upper():
                column = line.split()
                ...
                if len(column) >= 3:
                    if column[2] == 'Y':  # Checking if the third column is 'Y'
                        secret_dict[column[0]] = ['Secreted']
                        ph_s_out.write(f"{column[0]} is secreted\n")
                    elif int(column[1]) > 0:  # Checking if the second column has a positive integer
                        trans_dict[column[0]] = [column[1]]
                        ph_t_out.write(f"{column[0]} has {column[1]} transmembrane domains\n")
                else:
                    print(f"Skipping line due to insufficient columns: {line.strip()}")

    print(f"Phobius secreted and transmembrane data written to {phobius_secreted_output} and {phobius_transmembrane_output}")
    return
...
def parse_signalp(infile, anno_dict, isolate_dir):
    signalp_output = os.path.abspath(os.path.join(isolate_dir, 'signalp_output.txt'))

    with open(infile, 'r') as files, open(signalp_output, 'w') as sig_out:
        for line in files:
            if not line.startswith('#'):
                # Debug: Show the line being processed
                print(f"Processing line: {line.strip()}")
                columns = line.split()  # Correct variable name: columns
                # Debug: Show the parsed columns
                print(f"Columns: {columns}")

                # Check if the fourth column is 'signal_peptide'
                if columns[3] == 'signal_peptide':
                    protein_id = columns[0]  # Protein ID is in the first column
                    anno_dict[protein_id] = ['Secreted']
                    sig_out.write(f"{protein_id} is secreted by SignalP\n")
                    print(f"{protein_id} added as secreted")  # Debug: Show added proteins

    print(f"SignalP output written to {signalp_output}")
    return
...
def parse_annotations(infile, anno_dict):
    with open(infile, 'r') as files:
        for line in files:
            ...
            # For other annotations (e.g., 'note'), handle the possibility of colons in column[2]
            else:
                anno_list = anno_dict.get(column[0], [])
                
                # Check if column[2] contains a colon, and handle it accordingly
                if ':' in column[2]:
                    # Split on the first colon and take the second part
                    anno_dict[column[0]] = anno_list + [column[2].strip().split(':', 1)[1]]
                else:
                    # No colon, so take the entire third column as is
                    anno_dict[column[0]] = anno_list + [column[2].strip()]
    return
...
def parse_effectors(infile, anno_dict):
    """
    Parses the EffectorP output file to extract protein identifiers and their associated probabilities.

    Parameters:
    - infile (str): Path to the EffectorP output file.
    - anno_dict (dict): Dictionary to store protein identifiers and their probabilities.

    Returns:
    - None: Updates anno_dict in place.
    """
    with open(infile, 'r') as files:
        header = next(files)  # Skip the header line
        for line in files:
            ...
            cytoplasmic = columns[1]
            apoplastic = columns[2]
            non_effector = columns[3]
            prediction = columns[4]
            
            probability = None
            
            # Check if the prediction indicates an effector
            if 'Cytoplasmic effector' in prediction and cytoplasmic.startswith('Y'):
                # Extract probability from the Cytoplasmic effector column
                try:
                    probability = cytoplasmic.split('(')[1].strip(')')
                except IndexError:
                    probability = 'N/A'  # Handle unexpected format
            elif 'Apoplastic effector' in prediction and apoplastic.startswith('Y'):
                # Extract probability from the Apoplastic effector column
                try:
                    probability = apoplastic.split('(')[1].strip(')')
                except IndexError:
                    probability = 'N/A'  # Handle unexpected format
            
            if probability and probability != 'N/A':
                anno_dict[identifier] = [probability]
                print(f"Parsed effector: {identifier}, Probability: {probability}")
...
def main_analyses_loop(loop_dir, isolate, isolate_dir, iso_fasta_dir, iso_work_dir):
    ...
    for files in os.listdir(loop_dir): # loop through and parse annotation files
        ...
        # Ensure that you pass isolate_dir when calling parse_phobius
        if 'phobius' in analyses and 'phobius' in files.lower():
            phobius_file = os.path.abspath(os.path.join(loop_dir, files))
            parse_phobius(phobius_file, phobius_s, phobius_t, isolate_dir)
        # Ensure that you pass isolate_dir when calling parse_signalp
        if 'signalp' in analyses and 'signalp' in files.lower():
            signalp_file = os.path.abspath(os.path.join(loop_dir, files))
            parse_signalp(signalp_file, signalp, isolate_dir)
    ...
    if 'effectors' in analyses:
        ...
        # # Check if secretome_fasta was created
        #if os.path.exists(secretome_fasta):
        #    print(f"Secretome fasta file created successfully: {secretome_fasta}")
        #else:
        #    print(f"Failed to create secretome fasta file: {secretome_fasta}")
        if not os.path.exists(effector_out):
            with open(effector_log, 'w') as eff_log:
                subprocess.call(['python3', EFFECTORP, '-i', secretome_fasta, '-o', effector_out]
            ...
            if cat.lower()+'.tsv' in files:
                ...
                for type in type_dict.keys():
                    ...
                    if not type_df.empty:
                        ...
                        ax.set_yticklabels(y_ticks, fontsize=12, fontname = 'DejaVu Sans')
                        x_value = ax.get_xlim()
                        for i in range(len(y_ticks)):
                            text = ax.text(np.mean(x_value), i+0.5, go_ticks[i], ha='center', va='center', fontsize=10, fontname = 'DejaVu Sans')
                        ...
                        cbar.ax.set_title('-log10(P-value)', fontsize=12, fontname = 'DejaVu Sans')
                        plt.title('{} {}'.format(cat,type_dict[type]), fontsize=16, pad=10, fontname = 'DejaVu Sans')
                        plt.tight_layout()
...
def write_final_results_and_figure(loop_dir):
    ...
    for a in final_analyses:
        ...
        plt.xlabel('Pangenome categories', fontname = 'DejaVu Sans', fontsize=12)
        if a == 'metabolite':
            plt.ylabel(u'2\u00B0 metabolite protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'transmembrane':
            plt.ylabel('Transmembrane protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'secretome':
            plt.ylabel('Secreted protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'effector':
            plt.ylabel('Predicted effector protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'conserved':
            plt.ylabel('Protein clusters w/ conserved domains (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'MEROP':
            plt.ylabel('MEROP protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        elif a == 'CAZy':
            plt.ylabel('CAZy protein clusters (%)', fontname = 'DejaVu Sans', fontsize=12)
        plt.yticks(fontname = 'DejaVu Sans', fontsize=12)
        plt.xticks(fontname = 'DejaVu Sans', fontsize=12)
        plt.tight_layout()
